 ...
 */
export function useVoiceRecording({ isChat, onSend, onDismiss }: VoiceRecordingProps) {
    const {
        state: recorderState,
        error: recorderError,
        getAmplitude,
    ...
    React.useEffect(() => { getAmplitudeRef.current = getAmplitude }, [getAmplitude])
    React.useEffect(() => { getDurationRef.current = getDuration }, [getDuration])
    ...
    const showWaveform = recorderState === "recording" || recorderState === "paused"
        ...
        <>
            {showWaveform && (
                ...
                >
                    ...
                    <div className="flex flex-1 items-center justify-end gap-[2px] h-6 overflow-hidden">
                        {waveformBars.map((h, i) => (
                            <div
                                key={i}
                                className="w-[3px] shrink-0 rounded-[1.5px] bg-[#b76440] transition-all duration-75"
                                style={{ height: `${h > 0 ? Math.max(8, h) : 0}%`, opacity: h > 0 ? 1 : 0 }}
                            />
                        ))}
                    </div>
                    <span className="text-[14px] tabular-nums text-muted-foreground min-w-[36px] text-center shrink-0">
