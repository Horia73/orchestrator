...
export function SunArcInline({ sunrise, sunset, timezone, className }: SunArcProps) {
    ...
    return (
        <div className={cn("w-full", className)}>
            <svg viewBox="0 0 110 34" preserveAspectRatio="none" className="block h-[34px] w-full" aria-hidden>
                {/* Horizon — fine dashed line */}
                <line x1={2} y1={30} x2={108} y2={30} stroke="currentColor" strokeOpacity={0.22} strokeWidth={0.5} strokeDasharray="1.5 2.5" />
                ...
                {isVisible ? (
                    <>
                        {/* Halo */}
                        <circle cx={sunX} cy={sunY} r={5.5} fill="#fbbf24" fillOpacity={0.25} />
                        {/* Core */}
                        <circle cx={sunX} cy={sunY} r={2.6} fill="#fbbf24" stroke="white" strokeWidth={0.6} />
                    </>
                ) : (
                    <circle
                        cx={frac < 0 ? 8 : 102}
