...
export function AudioList({
    ...
    return (
        <ul className={cn("flex flex-col gap-1.5", className)} aria-label="Audio list">
            {attachments.map((a) => (
                <AudioRow key={a.id} attachment={a} />
            ))}
        </ul>
    )
}

function AudioRow({ attachment }: { attachment: LibraryAttachment }) {
    ...
    const progressPct = duration && duration > 0 ? (progressSec / duration) * 100 : 0

    return (
        <li className="overflow-hidden rounded-xl border border-border/55 bg-card shadow-sm">
            <div className="flex items-center gap-3 px-4 py-2.5">
                <button
                    type="button"
                    onClick={() => void togglePlay()}
                    aria-label={playing ? `Pause ${attachment.filename}` : `Play ${attachment.filename}`}
                    className={cn(
                ...
                <div className="min-w-0 flex-1">
                    <div className="flex flex-wrap items-baseline gap-x-2">
                        <span className="inline-flex items-center gap-1 text-sm font-medium text-foreground">
                            <Music className="size-3.5 text-muted-foreground" strokeWidth={1.85} aria-hidden />
                    ...
                    <div className="mt-1.5 flex items-center gap-2">
                        ...
                        {playing && duration !== null ? (
                            ...
                            <span className="shrink-0 text-[10.5px] text-muted-foreground">
                                {formatRelativeTime(attachment.messageTimestamp)} · <span className="truncate">{attachment.conversationTitle}</span>
                            </span>
                        )}
                    </div>
                </div>
            ...
            <audio ref={audioRef} src={`/api/uploads/${encodeURIComponent(attachment.id)}`} preload="metadata" />
        </li>
    )
}
