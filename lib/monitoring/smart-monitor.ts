...
export function buildSmartMonitorAgentPrompt(options: {
    ...
    lines.push('- Smart Monitor must have exactly one Scheduling runtime entry: this consolidated Smart monitor heartbeat. Do not create separate scheduled tasks for Smart Monitor digests, summaries, route-aware checks, source-specific wakeups, retries, or catch-up runs.')
    lines.push('- Store timed requirements as durable Smart Monitor preferences/specs in MONITORS.md, and store execution bookkeeping in this Smart Monitor task_state. On every heartbeat, use current runtime time to perform overdue still-useful work that has not already been completed, skipped, or deduplicated for the relevant period.')
    ...
    lines.push('- If MONITORS.md or MEMORY.md records a model-owned daily memory consolidation preference, and this wake happens after local midnight for a day not yet consolidated in task_state, you may also consolidate the just-ended MEMORY_DAY into durable memory. Suggested wall-clock times are guidance, not a hard schedule. Store bookkeeping under task_state.memoryMaintenance and stay silent unless there is an error.')
