...
function buildBriefPrompt(
    ...
    lines.push('Important operating model: this is one consolidated monitor wake, not one agent per source, recurring check, or urgency tier. You can evaluate many connector-backed and custom watches in this single turn. Group related items and notify sparingly.')
    ...
    lines.push('2) For things worth surfacing: call `notify_inbox` ONCE per logically distinct issue, with a specific email-style `title` that works as the Inbox subject. Group related matches across watches into a single message when they belong together. Be specific: who, what, value, link.')
