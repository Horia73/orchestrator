...
import { listWatchlistAlerts } from '@/lib/watchlist/store'

import { registerMarketsAdapter, type MarketsAdapter, type MonitoredInstrument } from './markets-heartbeat'
...
type Quoteish = { price?: number | null; previousClose?: number | null; changePercent?: number | null } | null
type ItemWithQuote = {
    id: string
    symbol: string
    name: string
...
const adapter: MarketsAdapter = {
    async snapshot() {
        ...
        const cfgRows = db.prepare(
            'SELECT id, movePercent, monitorEnabled FROM watchlist_items',
        ).all() as Array<{ id: string; movePercent: number | null; monitorEnabled: number | null }>
        const cfg = new Map(cfgRows.map(r => [r.id, r]))

        const instruments: MonitoredInstrument[] = (items as ItemWithQuote[]).map(it => {
            ...
            return {
                ...
                monitorEnabled: c ? c.monitorEnabled !== 0 : true,
        ...
        return { configured: status.configured, message: status.message, instruments, alerts }
    },
    ...
    appendObservation(o) {
        ...
        } catch { /* observation history is best-effort */ }
    },
