...
export interface MarketsAdapter {
    ...
    snapshot(): Promise<{ configured: boolean; message?: string; instruments: MonitoredInstrument[]; alerts: MonitoredAlert[] }>
    appendObservation(o: { instrumentId: string; providerSymbol: string; price: number | null; changePercent: number | null; ts: number }): void
...
export function registerMarketsAdapter(a: MarketsAdapter): void { adapter = a }
export function getMarketsAdapter(): MarketsAdapter | null { return adapter }
 ...
 */
export async function ensureMarketsHeartbeat(): Promise<void> {
    if (!adapter) return
    const { listScheduledTasks, createScheduledTask } = await import('@/lib/scheduling/store')
    const exists = listScheduledTasks().some(
...
export function assetClassDefaultMovePercent(assetClass: MonitoredInstrument['assetClass']): number {
    switch (assetClass) {
        case 'crypto': return 8
        case 'forex': return 2
        case 'index': return 1.5
        case 'etf': return 2.5
        case 'fund': return 2.5
        default: return 3 // stock / other
    }
}
...
function isMarketOpen(assetClass: MonitoredInstrument['assetClass'], now: number): boolean {
...
export async function runMarketsCheapPass(args: {
    ...
    const baseState: Record<string, unknown> = { ...prior, symbols, lastRunAt: args.now }

    if (!adapter) {
        return { noteworthy: false, summary: 'Markets monitor: watchlist data layer not wired yet — no-op.', nextState: baseState }
    }
    ...
    for (const inst of snap.instruments) {
        ...
        if (st.session !== session) { st.lastNotifiedPct = undefined; st.lastNotifiedAt = undefined; st.session = session }

        const threshold = inst.movePercent ?? assetClassDefaultMovePercent(inst.assetClass)
        ...
        if (pct != null && Math.abs(pct) >= threshold) {
            ...
            const movedFurther = last == null
                ? true
                : (pct > 0 && pct >= last + threshold / 2) || (pct < 0 && pct <= last - threshold / 2)
            if (movedFurther) {
                events.push(`${inst.symbol} (${inst.name}) ${pct > 0 ? '+' : ''}${pct.toFixed(2)}% today → ${inst.price} (notable-move threshold ${threshold}%)`)
    ...
    for (const alert of snap.alerts) {
        ...
        const crossed = alert.condition === 'above' ? inst.price >= alert.value : inst.price <= alert.value
        if (!crossed) continue
        // Don't re-fire the same alert within the same session.
        const firedThisSession = alert.lastTriggeredAt != null && sessionKey(alert.lastTriggeredAt) === session
        if (firedThisSession) continue
        events.push(`${inst.symbol} (${inst.name}) ${alert.condition} ${alert.value} → now ${inst.price} (your price alert)`)
