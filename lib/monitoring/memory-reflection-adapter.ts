...
const REFLECTION_PROMPT = [
  ...
  '5. For the Smart Monitor, review recent wake decisions with search_past_runs and consolidate durable learnings into MONITORS.md (recurring noise to keep quiet, recurring signals that matter, learned notify/quiet preferences); prune monitor notes that no longer hold.',
  '6. Never delete or rewrite the raw MEMORY_DAY/<date>.md day files — they are the audit trail and safety net. Curate only the durable files.',
  '7. Save changes with the Write/Edit tools (in-context edits do not persist). Keep every file compact. If nothing needs changing, change nothing and finish. Do NOT call notify_inbox.',
].join('\n')
