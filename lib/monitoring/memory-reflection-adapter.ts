...
const REFLECTION_PROMPT = [
  ...
  '1. Read your durable memory (USER.md, MEMORY.md, MONITORS.md, PLAYBOOKS.md) and the recent daily working memory already in your context. Also read MEMORY_ARCHIVE.md (it is NOT auto-loaded) before you reorganize the hot/cold split.',
  '2. Curate the durable files: resolve contradictions in favor of the newest evidence, merge duplicates — including the same fact duplicated across two files (e.g. in both USER.md and MEMORY.md): keep it only in its canonical home (profile/personal → USER.md, assistant operating rule → MEMORY.md, procedure → PLAYBOOKS.md) and delete the other copy; this is safe across the hot files but never demote an autonomous-run-needed fact into MEMORY_ARCHIVE.md to resolve a duplicate — and delete entries that are stale, superseded, or no longer relevant. Then DENSIFY — rewrite genuinely verbose or redundant entries to say the same thing in fewer words (same facts and nuance, tighter phrasing). Densification is lossless and is the preferred tool; deletion is lossy, so be conservative with it. Do NOT re-paraphrase already-tight prose night after night — repeated paraphrase risks meaning drift, so leave clean entries alone, and if a file is already lean change nothing. New facts are already saved during normal turns — your job here is cleanup, not bulk re-adding.',
  '3. Keep the HOT tier lean, but SAFELY. MEMORY.md is loaded into context every turn, so it should stay compact and high-signal — densification (step 2) is your main tool for that. You MAY also move durable facts that are rarely needed day-to-day into MEMORY_ARCHIVE.md (cold storage), but ONLY facts used in interactive conversation. CRITICAL: Smart Monitor wakes, scheduled tasks, and Microscript agent-wakes load MEMORY.md but do NOT run semantic recall, so a fact in the archive is INVISIBLE to them. Keep anything those autonomous runs depend on (notify/quiet rules, device/entity specifics they act on, output-schema requirements, standing auto-action authorizations) in MEMORY.md. Archiving is otherwise lossless — the archive is indexed for recall and resurfaces in chat. Read MEMORY_ARCHIVE.md (not auto-loaded) before reorganizing, and promote a fact back when it becomes routinely relevant again.',
  '4. Spot cross-day patterns: something recurring across several days in the daily memory (a sender, a request, a routine, a preference) that is now worth one durable line — add it to the right file. A single occurrence is not a pattern.',
  '5. Synthesize playbooks from repeated workflows: call memory_recent_activity (~14 days) and scan the daily-memory window for the same multi-step request recurring across distinct days. When a workflow recurred on 2-3+ distinct days and PLAYBOOKS.md has no entry covering it, distill one per <durable_procedure_protocol>: trigger phrase, ordered steps with the tools/integrations/sub-agents actually used, {{parameters}} for the run-specific values, preconditions and gotchas. Use memory_search to recover the concrete steps when the daily notes are thin. Merge near-duplicates instead of appending variants; delete playbooks that stopped working or stopped recurring. A single occurrence is not a playbook; simple one-tool asks need none.',
  '6. For the Smart Monitor, review recent wake decisions with search_past_runs and the per-watch engagement history (user_signal events in monitor_watch_get — what the user opened, replied to, or dismissed unread), and consolidate durable learnings into MONITORS.md (recurring noise to keep quiet, recurring signals that matter, learned notify/quiet preferences); prune monitor notes that no longer hold.',
  '7. Never delete or rewrite the raw MEMORY_DAY/<date>.md day files — they are the audit trail and safety net. Curate only the durable files.',
  '8. Save changes with the Write/Edit tools (in-context edits do not persist). Keep every file compact. If nothing needs changing, change nothing and finish. Stay silent — do NOT call notify_inbox — with ONE exception: if step 5 created a brand-new playbook, you may send a single short notify_inbox naming its trigger phrase(s) so the user knows the shortcut exists (and can correct or remove it). Edits, merges, and densification never notify.',
].join('\n')
