...
import type { Attachment, ContentSegment, ContextUsageSnapshot, ReasoningEntry } from '@/lib/types'
...
export interface ToolExecutionContext {
    ...
    onToolDelta?: (toolCallId: string, toolName: string, delta: ToolStreamDelta) => void | Promise<void>
 ...
 */
export type AgentKind = 'text' | 'image' | 'video' | 'speech' | 'music' | 'concierge' | 'phone' | 'android'
...
export interface AgentStreamEvent {
    type: 'thinking' | 'thinking_done' | 'content' | 'tool_call' | 'tool_delta' | 'tool_result' | 'delegate' | 'done' | 'error'
    content?: string
    seconds?: number
...
export interface StreamCallbacks {
    ...
    onToolDelta?: (toolCallId: string, toolName: string, delta: ToolStreamDelta) => void
    onToolResult: (toolCallId: string, toolName: string, result: ToolResult) => void
    ...
    onContextCompaction?: (event: { threadId?: string; turnId?: string; itemId?: string; at: number }) => void
    /**
     ...
     */
    onDone: (meta: { sessionId?: string; usage?: unknown; thinkingDuration?: number; attachments?: Attachment[] }) => void
...
export interface AIProvider {
    ...
    stream?(options: ProviderSendOptions, callbacks: StreamCallbacks): Promise<void>
