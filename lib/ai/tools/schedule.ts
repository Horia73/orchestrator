...
export const scheduleTaskTool: ToolDef = {
    ...
    input_schema: {
        ...
        properties: {
            ...
            when: {
                type: 'object',
                description: 'Exactly one timing field. Use timezone for daily/weekly/cron (IANA, e.g. "Europe/Bucharest"); defaults to the server timezone.',
                properties: {
                    at: { type: 'string', description: 'One-shot at an absolute time, ISO 8601 (e.g. "2026-05-18T21:00:00+03:00").' },
                    in: { type: 'string', description: 'One-shot after a relative delay: "90s", "30m", "7h", "3d".' },
                    daily_at: { type: 'string', description: 'Recurring every day at "HH:MM" (24h).' },
                    every: { type: 'string', description: 'Recurring fixed interval: "15m", "2h", "1d" (min 1m).' },
                    cron: { type: 'string', description: 'Recurring raw cron expression, e.g. "0 9 * * 1-5".' },
                    weekly_days: { type: 'array', items: { type: 'string' }, description: 'For weekly: days like ["mon","wed","fri"]. Pair with weekly_at.' },
        ...
        properties: {
            ...
            when: {
                ...
                properties: {
                    ...
                    every: { type: 'string', description: 'Recurring fixed interval: "15m","2h","1d".' },
