...
export const monitorWatchAddTool: ToolDef = {
    ...
    input_schema: {
        ...
        properties: {
            ...
            rule: { type: 'object', description: 'Structured MonitorRule: an object with a `kind` plus kind-specific fields. Field names are exact — e.g. gmail: {kind:"gmail_query", q:"in:inbox is:unread -in:spam"} (the field is `q`, NOT `query`); whatsapp: {kind:"wa_unread"}, with composable metadata leaves wa_message_type {types:[…]}, wa_has_text {value:boolean}, and wa_has_media {value:boolean}; calendar: {kind:"calendar_event_needs_response"}; custom: {kind:"custom_prompt", prompt:"…"}. Predicate kinds must match the source; use any_of / all_of for composition. Call monitor_describe_sources for the supported kinds per source.' },
