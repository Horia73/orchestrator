...
- for tunnel host selection, prefer the current app host or ssh_host_candidates from runtime_context/status over hardcoded IPs. Dynamic LAN IPs can change; if the app host is a stable DNS name such as orchestrator.lan, use that. If only an IP is available, use the current detected IP/candidate and tell the user to rerun status if it changes;
