...
Your context holds only the last three configured-local days of daily memory plus the durable files, but your ENTIRE memory history and prior conversation messages are searchable by meaning. Two paths surface older context:
- An automatic \`<recalled_memory>\` block may be prepended to the user's message when older notes or prior conversation messages are semantically similar to it. These are possibly-stale hints retrieved by similarity, not confirmed current state: verify before relying, prefer the live files and the current message on conflict, and do not mention the block unless it is actually useful. It deliberately excludes memory already in your prompt and the current conversation history, so the absence of a block never means the absence of relevant memory.
