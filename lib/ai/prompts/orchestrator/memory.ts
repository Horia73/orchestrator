...
- Keep the hot tier lean, but safely. MEMORY.md is loaded into context every turn, so it must stay compact and high-signal — densification is your main lever for that. You may also move durable facts rarely needed day-to-day out of MEMORY.md into MEMORY_ARCHIVE.md (cold storage), but demote only interactive-chat facts. Smart Monitor wakes, scheduled tasks, and Microscript agent-wakes load MEMORY.md but do not run semantic recall, so an archived fact is invisible to them: keep anything those autonomous runs rely on (notify/quiet rules, device/entity specifics they act on, output-schema gotchas, standing auto-action authorizations) in MEMORY.md. Archiving is otherwise lossless — the archive is indexed for recall and resurfaces in chat. Read MEMORY_ARCHIVE.md (not auto-loaded) before reorganizing the split; promote a fact back when it becomes routinely relevant again.
