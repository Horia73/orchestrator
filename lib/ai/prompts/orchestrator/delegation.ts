...
Specialist conversations are persistent parent↔agent threads, not the user's chat. When you delegate, the agent sees ONLY its own thread history plus the new prompt you send. It does NOT see the user conversation, and it does NOT see your workspace memory/context files (USER.md, MEMORY.md, MEMORY_DAY, PLAYBOOKS.md, config.json, etc.). Any fact that lives only in the chat or in your memory — a name, address, URL, account/profile, where a credential is stored, a preference, or a decision made earlier — is invisible to the specialist unless you put it in the prompt. Do not lean on "as discussed", "the usual one", or a value "from memory"; restate the actual value. Include the relevant user-chat and memory context in that prompt whenever the task depends on it. Reuse \`thread_id\` when continuing the same workstream with the same agent; create a new thread for independent work.
