...
export function buildRuntimeContext(ctx: PromptContext): string {
    ...
    if (serviceManager.toLowerCase() === 'docker') {
        lines.push('docker_runtime_topology: app=/app, source self-dev mount=/orchestrator-source, persistent state/workspace=/app/.orchestrator. Self-dev preview ports bind to container 127.0.0.1 and are user-facing only through /dev-preview/<run-id>/ on the Orchestrator origin. Standalone generated static apps persist under /published-apps/<slug>/ and publish-static may also return tailscaleFunnelUrl for the same path. Never return raw localhost ports as final URLs.')
        lines.push('host_reverse_proxy_policy: host nginx/HTTPS should normally proxy only to Orchestrator. Static generated apps use project_dev publish-static + /published-apps/<slug>/ + scoped Tailscale Funnel when available; nginx/systemd/Docker host changes require explicit approval and are fallback deploy work.')
    }
    ...
    lines.push('library_outputs: save user-kept documents, exports, media, and downloaded sources under `files/` in workspace_cwd. `files/`, `browser-downloads/`, `gmail-attachments/`, and `artifacts/` surface in Library → Files; static publishes under `published-apps/<slug>/` surface in Library → Artifacts → Webpages. Scratch elsewhere stays hidden. Generated HTML files for review can use `/files/<path>.html`. Interactive apps/sites/games must not use `/files`; use project_dev plus `project-run:run publish-static` for persistent `/published-apps/<slug>/` and report lanUrl/tailscaleFunnelUrl when returned.')
    ...
    lines.push('long_running_work: commands that outlive a reasonable wait (long builds, downloads, batch runs) go through `start_background_job`, then END your turn — the job survives it and its completion notice wakes you automatically. Native CLI Bash run_in_background dies with the turn; never rely on it. Inspect/kill via `manage_background_jobs`; when you expect the job to finish within minutes and want the result in THIS turn, use its `wait` action instead of sleep-and-poll loops.')
