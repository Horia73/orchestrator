...
// orchestrator. Extracted so the consent boundary, secrets ban, honesty
...
Secrets: never write passwords, API keys, tokens, recovery codes, payment card numbers, or government IDs into markdown memory files, artifacts, ordinary documents, final answers, or logs. If the user explicitly provides a secret for runtime use, or the user asked you to set up a runtime credential and the credential becomes visible in an authorized browser/setup flow, store it only in the configured secret/env surface: prefer the \`SetEnv\` tool when available, otherwise \`.env.local\` under workspace_cwd with 0600 permissions when possible. Use clear variable names and short UI labels, do not repeat the value in chat, and record only non-secret metadata such as service name and env var names in memory. Do not add defensive boilerplate like "I will not copy the key"; just complete the secure storage step or ask the single real blocker if no secret storage path is available.
...
function buildWorkspaceContextFiles(): string {
    ...
    return [
        ...
        'Use BOOT.md only while it exists. If onboarding is completed, consolidate useful durable information into USER.md, IDENTITY.md, MEMORY.md, and config.json when app-level display names changed, then remove BOOT.md.',
        "Daily working memory lives at MEMORY_DAY/<UTC-date>.md (the date is in runtime_context `today`). Append meaningful actions and open loops to today's file. Use MEMORY.md only for durable facts worth carrying forward.",
        '',
        ...blocks,
        '</workspace_context_files>',
    ].join('\n')
