...
function taskFromRow(row: ScheduledTaskRow): ScheduledTask {
    return {
        ...
        lastRunStatus: (row.lastRunStatus as ScheduledTask['lastRunStatus']) ?? null,
        lastRunError: row.lastRunError ?? null,
        lastConversationId: row.lastConversationId ?? null,
        runCount: row.runCount,
        consecutiveFailures: row.consecutiveFailures,
        createdBy: row.createdBy === 'orchestrator' ? 'orchestrator' : 'user',
...
export function listScheduledTasks(): ScheduledTask[] {
    const rows = db.prepare('SELECT * FROM scheduled_tasks ORDER BY createdAt DESC').all() as ScheduledTaskRow[]
...
export function updateScheduledTask(id: string, patch: UpdateScheduledTaskInput): ScheduledTask | null {
    ...
    if (!enabled) {
        status = 'paused'
        nextRunAt = null
    } else if (scheduleChanged || enabledChanged || current.status === 'paused' || current.status === 'missed') {
...
export function claimForRun(id: string, nowMs: number): ClaimedTask | null {
    const tx = db.transaction((): ClaimedTask | null => {
        const row = db.prepare('SELECT * FROM scheduled_tasks WHERE id = ?').get(id) as ScheduledTaskRow | undefined
        if (!row) return null
        if (row.enabled !== 1 || (row.status !== 'scheduled' && row.status !== 'error')) return null
...
export function finishRun(id: string, result: {
    ok: boolean
    isOnce: boolean
...
export function recordManualRun(id: string, result: {
    ok: boolean
    conversationId: string | null
    error?: string | null
    nowMs: number
...
export function markTaskError(id: string, message: string, nowMs: number): void {
    db.prepare(`
        UPDATE scheduled_tasks
...
export function recoverStuckRunning(nowMs: number): ScheduledTask[] {
    const rows = db.prepare("SELECT * FROM scheduled_tasks WHERE status = 'running'").all() as ScheduledTaskRow[]
...
export function recordTaskRun(run: {
    ...
    `).run({
        ...
        summary: run.summary.slice(0, 20_000),
        error: run.error ?? null,
    })
    // Keep only the most recent N per task.
    db.prepare(`
        DELETE FROM scheduled_task_runs
        WHERE taskId = @taskId AND id NOT IN (
            SELECT id FROM scheduled_task_runs WHERE taskId = @taskId
            ORDER BY startedAt DESC LIMIT @keep
        )
    `).run({ taskId: run.taskId, keep: RUN_HISTORY_KEEP })
}

export function listTaskRuns(taskId: string, limit = RUN_HISTORY_KEEP): TaskRunRecord[] {
    const rows = db.prepare(`
        SELECT * FROM scheduled_task_runs WHERE taskId = ? ORDER BY startedAt DESC LIMIT ?
    `).all(taskId, limit) as Array<Omit<TaskRunRecord, 'surfaced'> & { surfaced: number }>
    return rows.map(r => ({ ...r, surfaced: r.surfaced === 1, conversationId: r.conversationId ?? null, error: r.error ?? null }))
}
...
export function getTaskState(id: string): Record<string, unknown> | null {
    const row = db.prepare('SELECT state FROM scheduled_tasks WHERE id = ?').get(id) as { state: string | null } | undefined
...
export function setTaskState(id: string, state: unknown): void {
    ...
    db.prepare('UPDATE scheduled_tasks SET state = @state, updatedAt = @now WHERE id = @id')
...
export function createInboxConversation(args: {
    ...
    const tx = db.transaction(() => {
        ...
        `).run({ id: conversationId, title: args.title, createdAt: now, updatedAt: now, taskId: args.taskId })
        ...
        for (const msg of args.messages) {
            insertInboxMessage.run({
                ...
                role: msg.role,
                content: msg.content,
                contentSegments: msg.contentSegments ? JSON.stringify(msg.contentSegments) : null,
...
export function listInboxConversations(limit = 200): InboxListItem[] {
    const rows = db.prepare(`
        ...
        LIMIT ?
    `).all(limit) as Array<{
        id: string; title: string; createdAt: number; readAt: number | null
        scheduledTaskId: string | null; messageCount: number; preview: string | null
    }>
    return rows.map(r => ({
        id: r.id,
        title: r.title,
...
export function getInboxConversation(id: string): InboxConversationDetail | null {
    const row = db.prepare(
        "SELECT id, title, createdAt, readAt, scheduledTaskId FROM conversations WHERE id = ? AND origin = 'inbox'"
    ).get(id) as { id: string; title: string; createdAt: number; readAt: number | null; scheduledTaskId: string | null } | undefined
    if (!row) return null
    ...
    const msgRows = db.prepare(
        ...
        contentSegments: string | null; reasoning: string | null; thinking: string | null
        thinkingDuration: number | null; toolCalls: string | null; attachments: string | null; timestamp: number
    }>
...
export function deleteInboxConversation(id: string): boolean {
    const row = db.prepare("SELECT id FROM conversations WHERE id = ? AND origin = 'inbox'").get(id)
    if (!row) return false
