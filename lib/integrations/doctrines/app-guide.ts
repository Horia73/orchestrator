...
- Bundled styling/writing skills include theme-factory and internal-comms. Theme-factory applies curated color/font systems to decks, documents, PDFs, reports, and HTML artifacts; internal-comms loads format-specific guidance for 3P updates, newsletters, FAQs, status reports, leadership updates, project updates, and incident reports.
...
- Orchestrator-owned skills are different from provider-native skills/plugins: they are visible through <skills_index> and the SkillSearch / ActivateSkill / ReadSkillFile tools, gated by the profile's Skills permission, and usable by orchestrator + worker across Claude Code, Codex, and API providers.
