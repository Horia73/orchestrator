...
- application/vnd.ant.workout — interactive workout session. Tap a set to start a working-set timer in the bottom bar; Finish stops it and opens a compact editor for actual weight/reps/duration/RPE/notes; Save logs the set, starts the rest timer with chimes, and autosaves progress. Completed sets remain editable after Finish workout, and changed actuals/RPE/notes re-save the finished session. The exercise info button can show a longer description, alternatives, video, relevant term explanations, and a lazily fetched demo/equipment image. Finished sessions save a summary and land in Library → Workouts.
...
- In the artifact, a set is a timed action: tap set → bottom working timer counts up → Finish → edit actuals/notes → Save. Only Save marks the set complete and starts rest. Finish workout is blocked while a set is running or waiting to be saved, then a session summary appears at the bottom and persists to workspace/workouts. After Finish workout, completed/failed sets can still be edited from the set row/menu; edits re-save the same session instead of forcing a restart.
