...
- CLI accounts (bottom): Claude Code and Codex. Per CLI: install state, login state, Log in / Reconnect / Log out, "Use long-lived token" (Claude Code — non-expiring, best for headless), Restart, and a binary path. These are how the orchestrator delegates code work; no API key needed once logged in. When a CLI runtime exposes local skills, the agent can activate and follow relevant skill workflows: Codex skills are listed in the provider prompt, while Claude Code autodiscovers them natively (the Skill tool is enabled in its allowlist).
...
- CLI provider skills are separate from Orchestrator integration tools. Codex is queried through app-server skills/list for the active agent cwd; its returned skills are added to the runtime prompt, and an explicitly requested $skill also gets its SKILL.md attached to the Codex turn. Claude Code skills are handled natively by the claude CLI: the Skill built-in is included in the tool allowlist, so the CLI autodiscovers and progressively loads personal (user), project (.claude/skills), and installed plugin skills on its own — Orchestrator does not list them in the prompt. Skills are guidance/workflows the provider can read and follow; they are not a replacement for activating Orchestrator tools.
