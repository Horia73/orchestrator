...
<settings>
Seven tabs (Models, Auth, Files, Logs, Usage, Profiles, Updates). The light/dark Theme toggle is in the Settings header, top-right. The active tab is remembered. Settings is admin-only; member profiles are redirected back to the main app.
...
- CLI accounts (bottom): Claude Code and Codex. Per CLI: install state, login state, Log in / Reconnect / Log out, "Use long-lived token" (Claude Code — non-expiring, best for headless), Restart, and a binary path. These are how the orchestrator delegates code work; no API key needed once logged in. Native CLI skills/plugins are intentionally not exposed to Orchestrator agents; specialized workflows come through Orchestrator-owned tools/skills instead of Codex or Claude marketplace/plugin state.
