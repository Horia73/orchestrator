...
- application/vnd.ant.workout — interactive workout session. Tap the current next set to start a working-set timer in the bottom bar; Finish stops it and opens a compact editor for actual weight/reps/duration/RPE/notes; Save logs the set, starts the rest timer with chimes, and autosaves progress. Sets can be skipped from the row menu, and jumping ahead asks to mark the intervening sets skipped with an optional reason. Completed/failed/skipped sets remain editable after Finish workout, and changed actuals/RPE/notes re-save the finished session. The exercise info button can show a longer description, alternatives, video, relevant term explanations, and a lazily fetched demo/equipment image. Finished sessions save a summary and land in Library → Workouts.
