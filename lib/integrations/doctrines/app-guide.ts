...
- CLI provider skills are separate from Orchestrator integration tools. Codex is queried through app-server skills/list for the active agent cwd; its returned skills are added to the runtime prompt, and an explicitly requested $skill also gets its SKILL.md attached to the Codex turn. Claude Code skills are discovered from the filesystem locations Claude Code uses (user/project .claude/skills) and summarized in the runtime prompt. Skills are guidance/workflows the provider can read and follow; they are not a replacement for activating Orchestrator tools.
