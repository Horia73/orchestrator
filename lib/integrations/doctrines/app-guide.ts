...
- Browser agent is special: it always uses the Patchright browser behind the scenes, so Settings only expose the light model plus an optional pro model (escalates on hard blockers), each restricted to Google/Gemini or Codex CLI text models. Thinking controls follow the selected model; Media resolution appears only on Gemini slots. Browser evidence screenshots/videos are attached inline; on multi-site or separately verifiable flows the agent is expected to capture a screenshot at each verified milestone before moving on.
