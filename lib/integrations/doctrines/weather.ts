...
3. \`WeatherShow\` mounts the card INSTANTLY in normal chat turns — the chat route injects the artifact tag for you, so the live forecast (current, hourly, daily, detail tiles) is on screen the moment the tool returns. It returns \`directEmitted: true\`, \`identifier\`, and \`modelContext\`. The Outfit and "Why it feels this way" tiles render a "Working…" placeholder until you fill them. Do NOT emit an \`<artifact>\` tag yourself.
4. Fill the "Why" tile: use the compact \`modelContext\` to write 1-3 "Why it feels this way" rows and call \`WeatherSetWhy\` with the returned \`identifier\`. These rows are model-generated; ground every row in \`modelContext\`. Always read \`modelContext.localTime\`: if it is night or early morning, explain current comfort separately from later-today concerns, and mention UV only as a daytime/midday factor. The rows slot into the live card in place — no second card, no flicker.
5. Fill the "Outfit" tile: write a short, practical recommendation and call \`WeatherSetOutfit\` with the same \`identifier\`. Keep \`headline\` short ("Light jacket", "Umbrella worth carrying") and \`summary\` grounded in the actual temp/rain/wind/UV/AQI. It patches the visible card in place, exactly like WeatherSetWhy. If the user explicitly does not want clothing/why guidance, pass \`smartGuidance: false\` to \`WeatherShow\` up front so the base card mounts with no placeholders and you skip both setters.
...
7. Completing the Outfit + Why tiles IS the deliverable — no closing chat message is required. Add at most ONE short actionable line only when there is a genuinely useful highlight ("rain expected Friday afternoon", "warmest day is Wednesday at 28°"); otherwise stop once the tiles are filled. Never recap numbers the card already shows.
8. If \`suggestGoogleUpgrade\` is true AND you haven't already offered Google Weather in this conversation AND \`MEMORY.md\` doesn't contain a "user declined Google Weather offer" note, add a single short standalone line (this is the one case where a trailing line is worth it):
...
- \`smartGuidance\` (optional): default true in chat turns — the card mounts instantly with "Working…" placeholders on the Outfit + Why tiles, which you then fill via \`WeatherSetWhy\` + \`WeatherSetOutfit\`. Set false only when the user explicitly does not want clothing/why guidance; then the base card mounts with no placeholders and you skip both setters. (Legacy alias: \`deferDisplay: false\` has the same effect.)
...
<dont>
- **Don't emit an \`<artifact>\` tag for weather.** The chat route auto-injects the card the moment WeatherShow returns. Writing the tag yourself creates a duplicate card. You don't need any closing prose either — filling the Outfit + Why tiles is the deliverable.
