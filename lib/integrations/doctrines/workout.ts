...
\`\`\`
{
  ...
  groups: Array<{
    ...
    exercises: Array<{
      ...
      imageUrl?: string;                    // optional direct demo/equipment image URL
      imageQuery?: string;                  // optional image lookup query if no direct URL is known
...
- Read the user's logged comments, failed sets, actual weights/reps, RPE/RIR, and partial reps returned by \`GetExerciseHistory\`. Adapt the plan: repeat or reduce load after pain/form-breakdown notes, deload after repeated high-RPE failures, add load/reps only after clean completions, and avoid exercises the user explicitly noted as problematic unless you explain the substitution.
...
- For machine-heavy exercises, include \`description\` with setup details (seat height, pad alignment, handle path, range of motion) and either a stable \`imageUrl\` or a precise \`imageQuery\` like "leg press machine exercise setup". The renderer fetches a safe preview lazily when the user taps info.
