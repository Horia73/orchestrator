...
- **Demo images are a verified per-exercise library you build ONCE and reuse forever — do NOT rely on the renderer's blind auto-match (it routinely shows the wrong movement).** For each exercise, use the exercise-image tools keyed by the stable kebab-case \`id\`:
  1. Call \`GetExerciseImage({ exerciseId })\`. If \`found: true\`, set \`imageUrl\` to the returned URL and move on — the work is already done for this movement.
  2. If \`found: false\`, call \`SearchExerciseImages({ name, exerciseId, equipment })\`, then pick the ONE candidate that truly depicts this exact movement/machine. Match on the ExerciseDB candidate's canonical \`name\` + equipment (you cannot see the pixels through a tool result, so match deliberately by name — English movement names match best; for a setup you must confirm visually, hand the URL to \`browser_agent\`). Then call \`SaveExerciseImage({ exerciseId, url, name, note })\` so it is reused in every future session, and set \`imageUrl\` to that URL.
  3. If nothing clearly matches, leave \`imageUrl\` unset — a missing demo is better than a wrong one. Never invent image URLs.
  This is a per-exercise one-time cost: exercises you have already saved are instant on later sessions. Prefer a \`videoUrl\` for nuanced technique. The renderer's fuzzy ExerciseDB/library/web fallback still runs only for exercises with no saved image.
