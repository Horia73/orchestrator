...
// Claude Code's MCP startup timeout — a transient blip recovers on the first
// retry (localhost call), while a genuinely-down app fails in ~1.5s.
...
// to an empty list, which left Claude Code running with NO orchestrator tools —
// the model then called them by name and got "No such tool available". We retry
