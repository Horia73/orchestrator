...
const TYPE_REPAIR_HINTS: Record<string, string> = {
    'application/vnd.ant.workout': [
        ...
        '- Every numeric field (weightKg, weightPct, load, reps, restSec, durationSec, distanceM, rounds, rpe, rir) is a number, not a string.',
        '- A `weighted` set MUST have weightKg or weightPct; if the load is only bodyweight, change the exercise `kind` to "bodyweight".',
        '- A proprietary/non-kg machine uses exercise kind `resistance`, a required string `loadUnit` (for example "level"), and numeric planned-set `load`.',
