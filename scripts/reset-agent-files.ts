...
const files: Array<{ path: string; content: string; resetByDefault: boolean }> = [
    ...
    },
    {
        ...
        content: [
            ...
            '5. If the user starts a different conversation or task while onboarding is active, handle that task first. Then, in the same final response, always resume onboarding from the next unfinished stage in ONBOARDING.md with the next small group of questions. Do this after every unrelated task while BOOT.md still exists; do not silently postpone onboarding because the user changed the subject.',
            ...
            '7. If the user ignores the onboarding questions and asks about something else, that is not a skip or refusal: answer the new request, then ask the pending onboarding questions again at the end. Keep doing this on later turns until the user answers, onboarding is complete, or the user explicitly says to stop asking or leave them alone.',
            '8. If the user says skip/stop/not now, asks not to be asked again, or says to leave them alone about onboarding, force-finish onboarding: consolidate known durable facts, record missing non-blocking fields in ONBOARDING.md or MEMORY.md as "ask opportunistically later", set ONBOARDING.md Status to skipped, and remove BOOT.md. Mere silence, ignoring the questions, or changing the subject never authorizes removing BOOT.md.',
            '9. Ask follow-up questions only for genuine blockers or contradictions.',
            '',
            'Suggested stages:',
