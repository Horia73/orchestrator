 ...
 * these guarantees WITHOUT spinning up real agents (pure gate logic):
 *
 *   1. The total-active cap is never exceeded, no matter the tree shape.
 *   2. Top-level, nested, and provider-backed runs all share that same cap.
 *   3. Nested fan-out is DEADLOCK-FREE — a 1 → 10 → 5 tree completes even when
 *      the cap is far smaller than the tree (parents release their slot while
 *      awaiting children).
 *   4. Large trees are not rejected by a cumulative quota; they drain through
 *      the bounded active pool and complete under backpressure.
 *
...
// production RAM-derived default.
process.env.AGENT_MAX_RESIDENT_CODEX_PER_DEPTH = '2'

import {
    acquireRun,
    agentGateLimits,
    __setGlobalAgentCapForTest,
    getAgentGateStats,
...
async function simulateAgent(opts: {
    ...
    if (opts.isTopLevel) {
        liveTopLevel++
        if (liveTopLevel > maxTopLevel) maxTopLevel = liveTopLevel
    }
    try {
        ...
        if (opts.plan && opts.plan.count > 0) {
            ...
            if (children.length > 0) {
                ...
                leaveTotal()
                if (opts.isTopLevel) liveTopLevel--
...
async function main() {
    ...
    check('default global active-agent cap is exactly 12', agentGateLimits.total === 12, `cap=${agentGateLimits.total}`)

    // ---- Scenario 1: the user's tree, caps far smaller than the tree --------
    // 1 orchestrator → 10 children → each 5 (= 61 agents) under GLOBAL=3.
    // A naive "gate everything" semaphore would DEADLOCK here. Release-while-
    // waiting must let it complete with peak active ≤ caps.
    console.log('Scenario 1 — deep fan-out (1→10→5) under global cap=3:')
    __setGlobalAgentCapForTest(3)
    resetCounters()
    ...
    check('counters drained to zero', liveTotal === 0 && liveTopLevel === 0, `total=${liveTotal} topLevel=${liveTopLevel}`)

    // ---- Scenario 2: many top-level runs at once ----------------------------
    // 8 independent top-level runs (each delegating 4) under global=6. All six
    // available slots must be usable by top-level work; there is no main split.
    console.log('\nScenario 2 — top-level runs share the full global cap=6:')
    __setGlobalAgentCapForTest(6)
    resetCounters()
    ...
    check('top-level work can use all 6 global slots', maxTopLevel === 6, `peak=${maxTopLevel}`)
    check('counters drained to zero', liveTotal === 0 && liveTopLevel === 0, `total=${liveTotal} topLevel=${liveTopLevel}`)
    ...
    // ---- Scenario 4: providers do not partition active capacity ------------
    // Seven runs on the same backend must be able to fill all seven global
    // slots. The former Claude-specific cap of 5 no longer exists.
    console.log('\nScenario 4 — one provider can use the full global cap=7:')
    __setGlobalAgentCapForTest(7)
    let claudeLive = 0
    let claudeMax = 0
    ...
    check('provider-backed work never exceeded global cap (7)', claudeMax <= 7, `peak=${claudeMax}`)
    check('one provider reached all 7 global slots', claudeMax === 7, `peak=${claudeMax}`)
