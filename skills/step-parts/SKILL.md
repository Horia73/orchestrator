...
After downloading a `.step`/`.stp` file the user should keep, save it under the workspace `files/` directory (recommended: `files/cad/<part-slug>/`) so it surfaces in the Library, and link it in your response (`[part.step](files/cad/<part-slug>/part.step)`). When the part feeds CAD work, continue in the `cad` skill — its handoff renders the result in the in-app 3D viewer via an `application/vnd.ant.cad` artifact.
