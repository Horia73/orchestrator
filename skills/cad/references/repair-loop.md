...
### In-app 3D viewer artifact failure

Likely causes:

- no native GLB sidecar was exported for the primary part or assembly
- the artifact body references a path outside the workspace `files/` tree, or an absolute host path
- the GLB export step failed (read the `scripts/step` stderr)

Fix:

- rerun `scripts/step` with `--glb <name>.glb` and confirm the file exists under `files/cad/<part-slug>/`
- emit the `application/vnd.ant.cad` artifact again with workspace-relative paths
- if unresolved, report the failure and rely on CLI facts/measurements plus snapshot PNGs and download links for validation
