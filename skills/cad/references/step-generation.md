...
Every `scripts/step` run also writes hidden adjacent GLB/topology artifacts as part of the normal build. They power snapshot rendering and `scripts/inspect` refs, and are not optional in the STEP workflow. (They are distinct from the native GLB sidecar the in-app viewer uses.)
