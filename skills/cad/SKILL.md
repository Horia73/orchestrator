...
Use the CAD Python runtime: when the `CAD_PYTHON` environment variable is set run `"$CAD_PYTHON" scripts/step ...`, otherwise use `python3`. Treat `python` in examples as that interpreter placeholder. Use `python scripts/<tool> --help` for the complete current command interface; reference docs show recommended workflows, not every flag.
...
4. **Check named purchasable components.** When an assembly includes named off-the-shelf actuators, servos, motors, electronics boards, connectors, or other purchasable components, search the `step-parts` skill (activate it with ActivateSkill) before creating simplified placeholder geometry. If no exact match is found, record the miss and then use a documented envelope.
...
After completing CAD work that creates or modifies `.step`, `.stp`, `.stl`, `.3mf`, or native `.glb` artifacts, you must ALWAYS surface the result in the in-app 3D viewer:

1. Keep every deliverable under the workspace `files/` directory — recommended layout `files/cad/<part-slug>/` (generator `.py`, `.step`, sidecars, snapshots together). Files elsewhere in the workspace do not surface in the user's Library.
2. Export a native GLB sidecar for the primary part or assembly (`--glb <name>.glb` on the `scripts/step` run; see `references/supported-exports.md`) — the in-app viewer renders GLB only. Also export `.stl` and/or `.3mf` sidecars when the user wants to print or requested them.
3. Emit an `application/vnd.ant.cad` artifact in your final response so the user gets the interactive 3D viewer card in chat. Call ActivateIntegrationTools("cad") first if that schema is not already loaded in this conversation — the doctrine carries the exact JSON body. Reference the GLB and every downloadable file by workspace-relative path (e.g. `files/cad/bracket/bracket.glb`).
4. If GLB export or the artifact fails, report that and fall back to snapshot PNGs plus `[file](files/...)` download links instead of silently omitting the visual. This rule applies to every workflow in this skill, including secondary STL/3MF/GLB outputs.

When verification snapshots are generated, review them yourself before responding; mention the snapshot paths in the final response only when they add value beyond the live viewer. If no snapshot applies, or if snapshot generation fails, say why and report the deterministic validation that still ran.
...
Final responses should include the `application/vnd.ant.cad` viewer artifact, download links for generated files, validation actually run, assumptions, and caveats. Use `references/inspection-and-validation.md` for report structure.
