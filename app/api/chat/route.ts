...
function buildToolTitle(toolName: string, args: Record<string, unknown> | undefined): string {
    ...
    if (toolName === 'Bash' || toolName === 'shell') return typeof args?.command === 'string' ? `Run ${args.command.slice(0, 80)}` : 'Run command'
...
function buildFinalContextUsageSnapshot(args: {
    ...
    const visibleOutputTokens = args.provider === 'openai' && usage.outputTokens !== null && usage.thinkingTokens !== null
...
function mergeContextUsage(previous: ContextUsageSnapshot | null, next: ContextUsageSnapshot): ContextUsageSnapshot {
    ...
    return {
        ...
        threadTokens: next.threadTokens ?? (sameSource ? previous?.threadTokens : null) ?? null,
        lastCompactedAt: next.lastCompactedAt ?? (sameSource ? previous?.lastCompactedAt : null) ?? null,
        compactedCount: next.compactedCount ?? (sameSource ? previous?.compactedCount : undefined),
    }
}
...
function buildLocalAttachmentContext(attachments: Attachment[]): string {
    ...
    for (const att of attachments) {
        ...
        lines.push(`- ${filename} (${mimeType}, ${formatAttachmentSize(att.size)}); ${location}`)
    }
...
export async function POST(request: Request) {
    ...
    if (isUpdateMaintenanceActive()) {
        return new Response(
            JSON.stringify({ error: 'Update in progress. The app will reconnect after restart.' }),
            {
                status: 503,
                headers: {
    ...
    const resolvedOrchestratorSettings = getEffectiveAgentSettings(orchestrator.id)
    const agentSettings = resolvedOrchestratorSettings.fromOverride
        ? resolvedOrchestratorSettings
        : {
            ...resolvedOrchestratorSettings,
            provider: orchestrator.provider ?? resolvedOrchestratorSettings.provider,
    ...
    if (!providerStream) {
        return new Response(
            JSON.stringify({ error: `Provider ${agentSettings.provider} doesn't support text streaming` }),
    ...
    const toolSurface = resolveProviderToolSurface(candidateTools, orchestrator.builtins, provider.capabilities)
    ...
    if (!orchestrator.buildPrompt) {
        return new Response(
            JSON.stringify({ error: `Agent ${orchestrator.id} is missing buildPrompt` }),
    ...
    interface PendingArtifact { attrs: ArtifactOpenAttrs; content: string }
    ...
    let latestContextUsage: ContextUsageSnapshot | null = getConversation(conversationId)?.contextUsage ?? null
    let lastPublishedContextUsageKey = latestContextUsage ? contextUsageKey(latestContextUsage) : ''

    const persistAssistantProgress = (opts?: { force?: boolean; thinkingDuration?: number; status?: Message['status'] }) => {
        const force = opts?.force ?? false
        const now = Date.now()
        if (!force && now - lastProgressPersistAt < STREAM_PROGRESS_PERSIST_INTERVAL_MS) return
    ...
    const findAgentEntry = (runId: string) => accReasoning.find(
    ...
    const stringifyAgentToolResult = (result: { success: boolean; data?: unknown; error?: string }) => {
    ...
    const appendToolDelta = (
        ...
        const entry = accReasoning.find(item => item.type === 'tool_call' && item.toolCallId === toolCallId)
    ...
    const appendContextCompaction = (
        ...
        if (!accReasoning.some(entry => entry.type === 'context_compaction' && entry.id === entryId)) {
    ...
    const handleAgentEvent = (event: AgentRunEvent, send: (data: Record<string, unknown>) => void) => {
        if (event.type === 'agent_start') {
            ...
            if (entry?.type === 'agent_call') {
                const reasoning = entry.reasoning ?? []
                const exists = reasoning.some(item =>
                ...
                if (!exists) {
                    reasoning.push({
                        ...
                        title: event.toolCall.title ?? buildToolTitle(event.toolCall.name, event.toolCall.arguments),
                        content: '',
            ...
            const entry = findAgentEntry(event.runId)
            const toolEntry = entry?.type === 'agent_call'
                ? entry.reasoning?.find(item => item.type === 'tool_call' && item.toolCallId === event.toolCallId)
                : undefined
            ...
            if (entry?.type === 'agent_call') {
                ...
                if (event.thinkingDuration) entry.thinkingDuration = event.thinkingDuration
            }
        }
    ...
    const stream = new ReadableStream({
        async start(controller) {
            ...
            const publishContextUsage = (snapshot: ContextUsageSnapshot, opts?: { force?: boolean }) => {
                ...
                const persisted = updateConversationContextUsage(conversationId, enriched)
                send({ type: 'context_usage', contextUsage: persisted })
            }

            try {
                // Build messages with file attachments resolved from disk
                const includeLocalAttachmentContext = canProviderReadLocalUploads(agentSettings.provider)

                const resolvedMessages = messages.map(m => {
                    const messageAttachments = Array.isArray(m.attachments) ? m.attachments : []
                    const localAttachmentContext = includeLocalAttachmentContext && m.role === 'user'
                        ? buildLocalAttachmentContext(messageAttachments)
                        : ''
                    const messageContent = typeof m.content === 'string' ? m.content : ''
                    const result: { role: string; content: string; attachments?: MessageAttachment[] } = {
                        role: m.role,
                        content: appendPromptContext(messageContent, localAttachmentContext),
                    }

                    if (messageAttachments.length) {
                        const atts: MessageAttachment[] = []
                        for (const att of messageAttachments) {
                            if (!att || typeof att.id !== 'string' || typeof att.mimeType !== 'string') continue
                            // Only include files the provider supports natively
                            if (!isFileSupportedByProvider(agentSettings.provider, att.mimeType)) continue
                            const filePath = resolveExistingUploadPath(att.id)
                ...
                await providerStream.call(provider,
                    {
                        model: agentSettings.model,
                        ...
                        toolContext: {
                            ...
                            onToolDelta: (toolCallId, toolName, delta) => appendToolDelta(toolCallId, toolName, delta, send),
                    ...
                    },
                    {
                            ...
                            for (const ev of artifactParser.feed(text)) {
                                switch (ev.kind) {
                                    case 'prose':
                                        ...
                                        break
                                    case 'artifact_start':
                                        pendingArtifacts.set(ev.clientToken, { attrs: ev.attrs, content: '' })
                                        send({
                                            type: 'artifact_start',
                        ...
                        onToolCall(toolCall) {
                            ...
                            const safeArgs = redactToolArgs(toolCall.name, toolCall.arguments) ?? {}
                        ...
                        onToolResult(toolCallId, toolName, result) {
                            ...
                            const reasoningToolCall = accReasoning.find(
                                (entry) => entry.type === 'tool_call' && entry.toolCallId === toolCallId
                            )
                            const fallbackArgs = (reasoningToolCall?.type === 'tool_call' ? reasoningToolCall.args : undefined)
                                ?? ((result.data as Record<string, unknown>)?.path ? { path: (result.data as Record<string, unknown>).path } : {})
                            const displayText = reasoningToolCall?.type === 'tool_call'
                        ...
                        onContextCompaction(event) {
                            ...
                            publishContextUsage({
                                ...
                                contextWindow: latestContextUsage?.contextWindow ?? modelContextWindow,
                        ...
                        onDone(meta) {
                                ...
                                if (ev.kind === 'prose') {
                                    ...
                                    if (p) {
                                        try {
                                            ...
                                            send({ type: 'artifact_end', clientToken: ev.clientToken, artifact: row })
                                        } catch (err) {
                                            send({
                                                type: 'artifact_error',
                                                clientToken: ev.clientToken,
                            ...
                            if (meta.sessionId) {
                                updateInteractionId(conversationId, agentSettings.provider, meta.sessionId)
                            }
                                ...
                                publishContextUsage(
                                    agentSettings.provider === 'codex' && latestContextUsage
                                        ? {
                                            ...
                                            contextWindow: latestContextUsage.contextWindow ?? finalContextUsage.contextWindow,
                                            threadTokens: latestContextUsage.threadTokens ?? finalContextUsage.threadTokens,
                                            last: latestContextUsage.last ?? finalContextUsage.last,
                                            total: latestContextUsage.total ?? finalContextUsage.total,
                                            lastCompactedAt: latestContextUsage.lastCompactedAt ?? finalContextUsage.lastCompactedAt,
                                            compactedCount: latestContextUsage.compactedCount ?? finalContextUsage.compactedCount,
                                        }
                                        : finalContextUsage,
                                    { force: true }
                                )
                        ...
                        onError(error) {
                            ...
                            persistAssistantProgress({ force: true, thinkingDuration: 0, status: 'error' })
                ...
                addMessage(conversationId, {
                    id: messageId,
                    role: 'assistant',
                    content: aborted ? accContent : (accContent ? `${accContent}\n\n[Error: ${msg}]` : `[Error: ${msg}]`),
                ...
                send(aborted ? { type: 'stopped', messageId } : { type: 'error', error: msg })
            } finally {
