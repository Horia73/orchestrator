...
import { deleteInboxConversation, getInboxConversation, markInboxRead } from '@/lib/scheduling/store'
...
export async function GET(request: Request, { params }: { params: Promise<{ id: string }> }) {
    ...
    try {
        ...
        markInboxRead(id)
        return NextResponse.json({ item: { ...item, readAt: item.readAt ?? Date.now() } })
    } catch (error) {
        console.error('Failed to get inbox item', error)
        return NextResponse.json({ error: 'Failed to get inbox item' }, { status: 500 })
    }
}
...
export async function DELETE(request: Request, { params }: { params: Promise<{ id: string }> }) {
    ...
    try {
        ...
        if (!deleted) return NextResponse.json({ error: 'Inbox item not found' }, { status: 404 })
