    ...
    try {
        ...
        if (body?.content) {
            ...
            if (!agent) {
                ...
                appendInboxMessage(id, errorMsg)
                return NextResponse.json({ item: getInboxConversation(id) })
            }
            ...
            const doneByRun = new Map<string, Extract<AgentRunEvent, { type: 'agent_done' }>>()
            const parentCtx: ToolExecutionContext = {
                ...
                onAgentEvent: (event) => {
                    if (event.type === 'agent_start' && topRunId === null) topRunId = event.runId
            ...
            const assistantContent = result.success
                ? notifications.length > 0
                    ? notifications
                        ...
                        .join('\n\n---\n\n')
                    : String((result.data as { output?: unknown } | undefined)?.output ?? done?.content ?? '').trim() || '(no output)'
            ...
            const assistantMsg: Message = {
                ...
                replyActions: notifications.length > 0
                    ? notifications.flatMap((n) => n.actions ?? [])
                    : undefined,
                timestamp: Date.now(),
            }
            appendInboxMessage(id, assistantMsg)
