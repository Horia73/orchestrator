...
import { startRemoteMcpOAuth } from '@/lib/integrations/mcp'
...
export async function POST(request: Request) {
  return runWithRequestProfile(request, async () => {
    ...
    try {
      ...
      const result = await startRemoteMcpOAuth({
        serverId,
      ...
      return NextResponse.json(
        { error: err instanceof Error ? err.message : 'Could not start MCP OAuth.' },
        { status: 400 }
      )
    }
  })
