...
import { addWatchlistItem, listWatchlistAlerts, listWatchlistItems } from '@/lib/watchlist/store'
...
export async function GET(request: Request) {
    try {
        ...
        if (includeQuotes) {
            ...
            return NextResponse.json({ ...result, alerts: listWatchlistAlerts() }, { headers: NO_STORE })
        }
        ...
        return NextResponse.json({ error: 'Failed to list watchlist' }, { status: 500, headers: NO_STORE })
    }
}
...
export async function POST(request: Request) {
    ...
    try {
        ...
        const result = addWatchlistItem({
            ...
            providerSymbol: typeof body.providerSymbol === 'string' ? body.providerSymbol : undefined,
